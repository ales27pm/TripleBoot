...
AUTONOMOUS_SCRIPT="$REPO_ROOT/scripts/tripleboot_autonomous.sh"
...
UBUNTU_DISK="${UBUNTU_DISK:-}"
WINMAC_DISK="${WINMAC_DISK:-}"
USB_DISK="${USB_DISK:-}"
WINDOWS_ISO="${WINDOWS_ISO:-}"
WINDOWS_ISO_URL="${WINDOWS_ISO_URL:-}"
UBUNTU_VERSION="${UBUNTU_VERSION:-26.04}"
UBUNTU_EDITION="${UBUNTU_EDITION:-desktop}"
UBUNTU_ARCH="${UBUNTU_ARCH:-amd64}"
INCLUDE_OPENCORE="${INCLUDE_OPENCORE:-false}"
INCLUDE_OSX_KVM="${INCLUDE_OSX_KVM:-false}"
OPENCORE_SMBIOS="${OPENCORE_SMBIOS:-iMac20,1}"
OPENCORE_GPU_POLICY="${OPENCORE_GPU_POLICY:-disable-nvidia}"
AUTONOMOUS_PROFILE="${AUTONOMOUS_PROFILE:-$REPO_ROOT/profiles/pc-27pm.yml}"
AUTONOMOUS_OUTPUT="${AUTONOMOUS_OUTPUT:-$BUILD_DIR/autonomous-payload}"
DRY_RUN=false
FORCE=false
...
save_defaults() {
  ...
  tmp="$(mktemp)"
  {
    ...
    echo "# Generated by scripts/tripleboot_aio.sh save-defaults on $(date --iso-8601=seconds)"
    printf 'TRIPLEBOOT_WORKDIR=%q\n' "$WORKDIR"
    printf 'TRIPLEBOOT_BACKUP_ROOT=%q\n' "$BACKUP_ROOT"
    printf 'TRIPLEBOOT_INVENTORY_DIR=%q\n' "$INVENTORY_DIR"
    printf 'TRIPLEBOOT_BUILD_DIR=%q\n' "$BUILD_DIR"
    printf 'TRIPLEBOOT_DOWNLOAD_DIR=%q\n' "$DOWNLOAD_DIR"
    printf 'TRIPLEBOOT_LOG_DIR=%q\n' "$LOG_DIR"
    printf 'EFI_SIZE=%q\n' "$EFI_SIZE"
    printf 'WINDOWS_SIZE=%q\n' "$WINDOWS_SIZE"
    printf 'UBUNTU_SWAP_SIZE=%q\n' "$UBUNTU_SWAP_SIZE"
    printf 'UBUNTU_DISK=%q\n' "$UBUNTU_DISK"
    printf 'WINMAC_DISK=%q\n' "$WINMAC_DISK"
    printf 'USB_DISK=%q\n' "$USB_DISK"
    printf 'WINDOWS_ISO=%q\n' "$WINDOWS_ISO"
    printf 'WINDOWS_ISO_URL=%q\n' "$WINDOWS_ISO_URL"
    printf 'UBUNTU_VERSION=%q\n' "$UBUNTU_VERSION"
    printf 'UBUNTU_EDITION=%q\n' "$UBUNTU_EDITION"
    printf 'UBUNTU_ARCH=%q\n' "$UBUNTU_ARCH"
    printf 'INCLUDE_OPENCORE=%q\n' "$INCLUDE_OPENCORE"
    printf 'INCLUDE_OSX_KVM=%q\n' "$INCLUDE_OSX_KVM"
    printf 'OPENCORE_SMBIOS=%q\n' "$OPENCORE_SMBIOS"
    printf 'OPENCORE_GPU_POLICY=%q\n' "$OPENCORE_GPU_POLICY"
    printf 'AUTONOMOUS_PROFILE=%q\n' "$AUTONOMOUS_PROFILE"
    printf 'AUTONOMOUS_OUTPUT=%q\n' "$AUTONOMOUS_OUTPUT"
    printf 'OSX_KVM_DIR=%q\n' "${OSX_KVM_DIR:-$(osx_kvm_dir_default)}"
  } > "$tmp"
  chmod 600 "$tmp"
  mv "$tmp" "$dest"
...
show_defaults() {
  tripleboot_banner
  cat <<EOF_DEFAULTS
TripleBoot saved defaults

Defaults file: $DEFAULTS_FILE
Exists: $([[ -f "$DEFAULTS_FILE" ]] && echo yes || echo no)
...
Disks and installers:
  Ubuntu disk:   ${UBUNTU_DISK:-not set}
  Win/mac disk:  ${WINMAC_DISK:-not set}
  USB disk:      ${USB_DISK:-not set}
  Windows ISO:   ${WINDOWS_ISO:-not set}
  Windows URL:   ${WINDOWS_ISO_URL:+set}

Layout:
  EFI size:      $EFI_SIZE
  Windows size:  $WINDOWS_SIZE
  Ubuntu swap:   $UBUNTU_SWAP_SIZE

USB build:
  Ubuntu:        $UBUNTU_VERSION $UBUNTU_EDITION $UBUNTU_ARCH
  OpenCore:      $INCLUDE_OPENCORE
  OSX-KVM:       $INCLUDE_OSX_KVM
  SMBIOS/GPU:    $OPENCORE_SMBIOS / $OPENCORE_GPU_POLICY

Autonomous profile:
  Profile:       $AUTONOMOUS_PROFILE
  Output:        $AUTONOMOUS_OUTPUT
EOF_DEFAULTS
}

prompt_value() {
  local label="$1" var_name="$2" current="${!2}" answer=""
  printf '%s [%s]: ' "$label" "${current:-blank}"
  read -r answer
  if [[ -n "$answer" ]]; then
...
prompt_bool() {
  local label="$1" var_name="$2" current="${!2}" answer=""
  printf '%s (true/false) [%s]: ' "$label" "$current"
  read -r answer
  case "${answer,,}" in
    "") ;;
    1|yes|y|true|on) printf -v "$var_name" '%s' true ;;
    0|no|n|false|off) printf -v "$var_name" '%s' false ;;
    *) warn "Invalid boolean '$answer'; keeping $current" ;;
  esac
}

configure_defaults() {
  [[ "$NONINTERACTIVE" == true ]] && die "configure-defaults requires an interactive terminal."
  tripleboot_banner
  echo "Configure reusable TripleBoot defaults. Press Enter to keep each current value."
  echo
  prompt_value "Ubuntu target disk" UBUNTU_DISK
  prompt_value "Windows/macOS target disk" WINMAC_DISK
  prompt_value "Installer USB disk" USB_DISK
  prompt_value "Windows ISO path" WINDOWS_ISO
  prompt_value "Windows ISO URL (optional)" WINDOWS_ISO_URL
  prompt_value "Ubuntu version" UBUNTU_VERSION
  prompt_value "Ubuntu edition" UBUNTU_EDITION
  prompt_value "Ubuntu architecture" UBUNTU_ARCH
  prompt_value "EFI partition size" EFI_SIZE
  prompt_value "Windows partition size" WINDOWS_SIZE
  prompt_value "Ubuntu swapfile size" UBUNTU_SWAP_SIZE
  prompt_bool "Include OpenCore in full USB build" INCLUDE_OPENCORE
  prompt_bool "Include OSX-KVM assets in full USB build" INCLUDE_OSX_KVM
  prompt_value "OpenCore SMBIOS" OPENCORE_SMBIOS
  prompt_value "OpenCore GPU policy" OPENCORE_GPU_POLICY
  prompt_value "Autonomous profile" AUTONOMOUS_PROFILE
  prompt_value "Autonomous output" AUTONOMOUS_OUTPUT
  prompt_value "Work directory" WORKDIR
...
require_default() {
  local var_name="$1" label="$2" value="${!1}"
  [[ -n "$value" ]] || die "Missing saved default for $label. Run: $0 configure-defaults"
}

run_default_preflight_partition() {
  require_default UBUNTU_DISK "Ubuntu disk"
  require_default WINMAC_DISK "Windows/macOS disk"
  preflight_partition --ubuntu-disk "$UBUNTU_DISK" --winmac-disk "$WINMAC_DISK"
}

run_default_partition() {
  require_default UBUNTU_DISK "Ubuntu disk"
  require_default WINMAC_DISK "Windows/macOS disk"
  partition_cmd --ubuntu-disk "$UBUNTU_DISK" --winmac-disk "$WINMAC_DISK" --yes-destroy
}
...
run_default_build_usb() {
  ...
  if [[ -n "$WINDOWS_ISO_URL" ]]; then
    ...
    require_default WINDOWS_ISO "Windows ISO path or URL"
    args+=(--windows-iso "$WINDOWS_ISO")
  fi
  if bool_flag "$INCLUDE_OPENCORE"; then
    args+=(--include-opencore --opencore-smbios "$OPENCORE_SMBIOS" --opencore-gpu-policy "$OPENCORE_GPU_POLICY")
  fi
  if bool_flag "$INCLUDE_OSX_KVM"; then
    args+=(--include-osx-kvm)
  fi
  build_tripleboot_usb "${args[@]}"
}
...
run_default_opencore() {
  download_opencore
  download_kexts
  build_opencore_scaffold --smbios "$OPENCORE_SMBIOS" --gpu-policy "$OPENCORE_GPU_POLICY"
  validate_opencore || true
}

run_autonomous_wrapper() {
  [[ -f "$AUTONOMOUS_SCRIPT" ]] || die "Autonomous wrapper not found: $AUTONOMOUS_SCRIPT"
  bash "$AUTONOMOUS_SCRIPT" "$@"
}

run_default_autonomous_validate() {
  run_autonomous_wrapper validate --profile "$AUTONOMOUS_PROFILE"
}

run_default_autonomous_generate() {
  run_autonomous_wrapper generate --profile "$AUTONOMOUS_PROFILE" --output-dir "$AUTONOMOUS_OUTPUT"
}

run_default_autonomous_stage_usb() {
  require_default USB_DISK "USB disk"
  run_autonomous_wrapper stage-usb --usb-disk "$USB_DISK" --profile "$AUTONOMOUS_PROFILE" --output-dir "$AUTONOMOUS_OUTPUT"
}

run_default_autonomous_status_usb() {
  require_default USB_DISK "USB disk"
  run_autonomous_wrapper status-usb --usb-disk "$USB_DISK"
}
...
menu() {
  [[ "$NONINTERACTIVE" == true ]] && die "menu requires an interactive terminal. Use direct commands with --noninteractive."
  local choice=""
  while true; do
    tripleboot_banner
    cat <<EOF_MENU
TripleBoot simple menu
Defaults: $DEFAULTS_FILE

Planning and safety:
  1) Show plan
  2) Check installer dependencies
  3) Scan hardware
  4) Analyze scan
  5) Doctor / safety report
  6) Backup EFI partitions
  7) Boot report

Saved defaults:
  8) Show saved defaults
  9) Configure saved defaults

USB / installers:
 10) USB build plan
 11) Build full TripleBoot USB using saved defaults
 12) USB status using saved default USB disk

Disk operations:
 13) Preflight partition check using saved defaults
 14) Partition target disks using saved defaults (destructive)
 15) Setup Ubuntu swapfile using saved default size

OpenCore / macOS lab:
 16) Download/build/validate OpenCore scaffold using saved defaults
 17) OSX-KVM doctor

Autonomous profile payload:
 18) Validate autonomous profile
 19) Generate autonomous payload
 20) Stage autonomous payload to saved default USB
 21) Autonomous USB status
...
EOF_MENU
    ...
    case "$choice" in
      ...
      7) boot_report ;;
      8) show_defaults ;;
      9) configure_defaults ;;
      10)
        local -a usb_plan_args=()
        bool_flag "$INCLUDE_OPENCORE" && usb_plan_args+=(--include-opencore)
      ...
      17) osx_kvm_doctor ;;
      18) run_default_autonomous_validate ;;
      19) run_default_autonomous_generate ;;
      20) run_default_autonomous_stage_usb ;;
      21) run_default_autonomous_status_usb ;;
...
Saved-default shortcuts:
  default-preflight-partition
  default-partition --yes-destroy
  default-build-tripleboot-usb --yes-destroy
  default-opencore
  default-autonomous-validate
  default-autonomous-generate
  default-autonomous-stage-usb
  default-autonomous-status-usb
...
Saved defaults:
  Defaults file: $DEFAULTS_FILE
  Start here:    sudo -E scripts/tripleboot_aio.sh menu
EOF_USAGE
}
...
main() {
  ...
  case "$cmd" in
    ...
    save-defaults) save_defaults ;;
    default-preflight-partition) run_default_preflight_partition ;;
    default-partition) default_partition_cmd "$@" ;;
    default-build-tripleboot-usb) default_build_usb_cmd "$@" ;;
    default-opencore) run_default_opencore ;;
    default-autonomous-validate) run_default_autonomous_validate ;;
    default-autonomous-generate) run_default_autonomous_generate ;;
    default-autonomous-stage-usb) run_default_autonomous_stage_usb ;;
    default-autonomous-status-usb) run_default_autonomous_status_usb ;;
    plan) plan ;;
